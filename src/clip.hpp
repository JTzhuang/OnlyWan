...
// Runner wrapper for CLIPVisionModelProjection — owns backend + params buffer,
// loads weights via ModelLoader, exposes compute() for I2V clip_fea encoding.
struct CLIPVisionModelProjectionRunner : public GGMLRunner {
    CLIPVisionModelProjection model;

    CLIPVisionModelProjectionRunner(ggml_backend_t backend,
                                    bool offload_params_to_cpu,
                                    const String2TensorStorage& tensor_storage_map,
                                    const std::string prefix,
                                    ...
                                    bool proj_in               = false)
        : GGMLRunner(backend, offload_params_to_cpu) {
        model = CLIPVisionModelProjection(version, transpose_proj_w, proj_in);
        model.init(params_ctx, tensor_storage_map, prefix);
    }
    ...
    struct ggml_tensor* forward(GGMLRunnerContext* ctx,
                                ...
                                bool return_pooled = false,
                                int clip_skip      = -1) {
        return model.forward(ctx, pixel_values, return_pooled, clip_skip);
    }
    ...
    struct ggml_cgraph* build_graph(struct ggml_tensor* pixel_values,
        ...
        auto runner_ctx        = get_context();
        struct ggml_tensor* out = forward(&runner_ctx, pixel_values, return_pooled, clip_skip);
