#ifndef __WAN_AVI_WRITER_H__
#define __WAN_AVI_WRITER_H__
...
/**
 * Create an MJPG AVI file from an array of wan_image_t images.
 ...
 */
int create_mjpg_avi_from_wan_images(const char* filename, wan_image_t* images, int num_images, int fps, int quality = 90) {
    if (num_images == 0) {
    ...
    uint32_t channels = images[0].channels;
    ...
    wan_avi_index_entry* index = (wan_avi_index_entry*)malloc(sizeof(wan_avi_index_entry) * num_images);
    if (!index) {
        fclose(f);
        return -1;
    }
