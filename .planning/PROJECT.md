...
## Current Milestone: v1.1 模型格式扩展

**Goal:** 支持 safetensors 格式模型加载，修复 v1.0 遗留问题，提升可用性

**Target features:**
- safetensors 运行时直接加载（无需预转换）
- safetensors → GGUF 转换工具
- progress_cb 在生成循环中实际触发
- 移除遗留 stub（wan_generate_video_t2v/i2v）
- 词汇表改为 mmap I/O（消除 ~85MB 头文件）
