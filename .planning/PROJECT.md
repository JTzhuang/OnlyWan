...
## Current Milestone: v1.2 性能优化与多卡推理 (SHIPPED 2026-03-26)

**Delivered:** 性能优化（CUDA Graph、算子融合）+ 多卡推理支持（张量并行、数据并行）+ spdlog 日志系统集成

**Key features:**
- ✓ CUDA Graph 和算子融合（2-5x 去噪加速）
- ✓ 多卡推理支持（张量并行 + 数据并行）
- ✓ spdlog 日志系统集成（动态级别控制）
- ✓ safetensors 运行时加载 + 转换工具
- ✓ 词汇表 mmap I/O（消除 ~85MB 头文件）
